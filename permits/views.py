...
def export_permit_reports_excel(request):
    ...
    if report_type in {"workers_at_work", "active_permissions"}:
        ws.append([
            ...
            "Permit Reference", "Destination", "Permission Starts", "Permission Ends",
        ])
        workforce_rows = (
        ...
        for row in workforce_rows:
            ...
            ws.append([
                ...
                (
                    _format_local_datetime(permit_request.start_time)
...
def export_permit_reports_pdf(request):
    ...
    report_pagesize = A4 if report_type == "workers_at_work" else landscape(A4)
    report_labels = {
        "workers_at_work": "Workers at Work",
        "active_permissions": "Workers with Active Permission",
    }
    report_label = report_labels.get(
    ...
    elements.append(Paragraph(
        (
            ...
            else "Permit Decision-Making Report"
        ),
        title_style,
    ))
    elements.append(Paragraph(f"<b>Report Type:</b> {report_label}", normal_style))
    ...
    if report_type in {"workers_at_work", "active_permissions"}:
        ...
        data = (
            ...
            if report_type == "workers_at_work"
                ...
                "Worker", "Employee ID", "Unit", "Department",
                "Permit Reference", "Destination", "Permission Starts",
                "Permission Ends",
            ]]
        )
        workforce_rows = (
        ...
        for row in workforce_rows:
            ...
            if report_type == "active_permissions":
                worker_cells.extend([
                    permit_request.reference_no,
                    permit_request.destination,
                    _format_local_datetime(permit_request.start_time),
    ...
    workforce_column_widths = {
        "workers_at_work": [145, 80, 190, 75],
        "active_permissions": [105, 62, 105, 55, 120, 100, 105, 105],
    }
    table = Table(
        table_data,
        repeatRows=1,
        hAlign="LEFT",
        colWidths=workforce_column_widths.get(report_type),
    )
    ...
    def add_page_number(canvas, doc):
        ...
        canvas.drawRightString(report_pagesize[0] - 25, 18, page_text)
        canvas.drawString(25, 18, f"{report_label} Report")
        canvas.restoreState()
