...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        sections = {
            ...
            "B": configure_section(form, 2, "SEHEMU B: TATHMINI YA MAANDALIZI NA UENDESHAJI WA MAONESHO", "SECTION B: EXHIBITION ORGANIZATION AND OPERATIONS", "Tafadhali tathmini maeneo yafuatayo kwa kutumia kipimo cha 1–5, ambapo 1 = Duni sana, 2 = Duni, 3 = Wastani, 4 = Nzuri, 5 = Nzuri sana.", "Please rate the following areas using the 1–5 scale, where 1 = Very poor, 2 = Poor, 3 = Fair, 4 = Good, 5 = Very good."),
            "C": configure_section(form, 3, "SEHEMU C: MANUFAA NA MATOKEO YA USHIRIKI", "SECTION C: PARTICIPATION BENEFITS AND OUTCOMES", "Ushiriki wa taasisi yako katika maonesho umefanikisha kwa kiwango gani matokeo yafuatayo? Ambapo 1 = Haijafikiwa, 2 = Kidogo, 3 = Wastani, 4 = Imefikiwa, 5 = Imefikiwa sana.", "To what extent did your institution's participation achieve the following outcomes? Where 1 = Not achieved, 2 = Slightly, 3 = Moderate, 4 = Achieved, 5 = Highly achieved."),
        ...
        configure_question(form, sections["A"], label_en="A1. Approximately how many people visited your booth during the exhibition?", label_sw="A1. Takribani watu wangapi walitembelea banda lako katika kipindi chote cha maonesho?", question_type=FormQuestion.QuestionType.SINGLE_CHOICE, order=1, required=True, options=visitor_ranges, existing_label_en="Approximately how many people visited your booth during the exhibition?")
        configure_question(form, sections["A"], label_en="A2. How would you rate the public response in visiting your booth?", label_sw="A2. Kwa mtazamo wako, kiwango cha mwitikio wa wananchi katika kutembelea banda lako kilikuwaje?", question_type=FormQuestion.QuestionType.SINGLE_CHOICE, order=2, required=True, options=response_rating, existing_label_en="How would you rate the public response in visiting your booth?")
        configure_question(form, sections["A"], label_en="A3. How interested were visitors in the products, services, technologies or innovations displayed?", label_sw="A3. Wageni waliotembelea banda lako walionesha kiwango gani cha kuvutiwa na bidhaa, huduma, teknolojia au ubunifu mlioonesha?", question_type=FormQuestion.QuestionType.SINGLE_CHOICE, order=3, required=True, options=interest_rating, existing_label_en="How interested were visitors in the products, services, technologies or innovations displayed?")
        visitor_group_question = configure_question(form, sections["A"], label_en="A4. Which groups visited your booth most? (Select all that apply)", label_sw="A4. Ni makundi gani yalitembelea zaidi banda lako? (Unaweza kuchagua zaidi ya moja)", question_type=FormQuestion.QuestionType.MULTIPLE_CHOICE, order=4, required=True, options=visitor_groups, existing_label_en="Which groups visited your booth most? (Select all that apply)")
        configure_question(form, sections["A"], label_en="A5. Other visitor group (specify)", label_sw="A5. Wengine, taja", question_type=FormQuestion.QuestionType.SHORT_TEXT, order=5, required=True, condition_question=visitor_group_question, condition_value="OTHER", existing_label_en="Other visitor group (specify)")
        ...
        for order, (sw, en) in enumerate(d_items, 1):
            configure_question(form, sections["C"], label_en=f"C{order}. {en}", label_sw=f"C{order}. {sw}", question_type=FormQuestion.QuestionType.SINGLE_CHOICE, order=order, required=True, options=achievement_rating, existing_label_en=f"C1.{order} {en}")
        partnership = configure_question(form, sections["C"], label_en="C9. Were any important new collaborations, opportunities or contacts obtained?", label_sw="C9. Je, kuna ushirikiano, fursa au mawasiliano mapya muhimu yaliyopatikana kutokana na maonesho haya?", question_type=FormQuestion.QuestionType.SINGLE_CHOICE, order=9, required=True, options=(("YES", "Ndiyo", "Yes"), ("NO", "Hapana", "No")), existing_label_en="Were any important new collaborations, opportunities or contacts obtained?")
        configure_question(form, sections["C"], label_en="C10. If yes, briefly explain", label_sw="C10. Kama Ndiyo, eleza kwa kifupi", question_type=FormQuestion.QuestionType.LONG_TEXT, order=10, required=True, condition_question=partnership, condition_value="YES", existing_label_en="If yes, briefly explain")
        configure_question(form, sections["C"], label_en="C11. To what extent did participation meet your institution's expectations?", label_sw="C11. Kwa ujumla, ushiriki katika maonesho haya umekidhi matarajio ya taasisi yako kwa kiwango gani?", question_type=FormQuestion.QuestionType.SINGLE_CHOICE, order=11, required=True, options=achievement_rating, existing_label_en="To what extent did participation meet your institution's expectations?")
        ...
        configure_question(form, sections["D"], label_en="D1. Overall satisfaction with exhibition organization and operations", label_sw="D1. Kwa ujumla, umeridhika kwa kiwango gani na maandalizi na uendeshaji wa maonesho?", question_type=FormQuestion.QuestionType.SINGLE_CHOICE, order=1, required=True, options=satisfaction, existing_label_en="Overall satisfaction with exhibition organization and operations")
        configure_question(form, sections["D"], label_en="D2. Would your institution participate again in future exhibitions?", label_sw="D2. Je, ungependa taasisi yako kushiriki tena katika maonesho yajayo?", question_type=FormQuestion.QuestionType.SINGLE_CHOICE, order=2, required=True, options=future_participation, existing_label_en="Would your institution participate again in future exhibitions?")
        configure_question(form, sections["D"], label_en="D3. Would you recommend that other institutions participate in future exhibitions?", label_sw="D3. Je, ungependekeza taasisi nyingine kushiriki katika maonesho yajayo?", question_type=FormQuestion.QuestionType.SINGLE_CHOICE, order=3, required=True, options=recommendation, existing_label_en="Would you recommend that other institutions participate in future exhibitions?")
        ...
        configure_question(form, sections["E"], label_en="E1. List the three main achievements gained from participating", label_sw="E1. Taja mafanikio makuu matatu ambayo taasisi yako imeyapata kutokana na kushiriki katika maonesho haya", question_type=FormQuestion.QuestionType.LONG_TEXT, order=1, required=True, existing_label_en="List the three main achievements gained from participating")
        challenges = configure_question(form, sections["E"], label_en="E2. Main challenges encountered (select all that apply)", label_sw="E2. Ni changamoto zipi kuu mlizokutana nazo wakati wa maandalizi na ushiriki katika maonesho? (Chagua zote zinazohusika)", question_type=FormQuestion.QuestionType.MULTIPLE_CHOICE, order=2, required=True, options=challenge_options, existing_label_en="Main challenges encountered (select all that apply)")
        configure_question(form, sections["E"], label_en="E3. Other challenge (specify)", label_sw="E3. Changamoto nyingine, taja", question_type=FormQuestion.QuestionType.SHORT_TEXT, order=3, required=True, condition_question=challenges, condition_value="OTHER", existing_label_en="Other challenge (specify)")
        configure_question(form, sections["E"], label_en="E4. Which was the greatest challenge?", label_sw="E4. Kati ya changamoto ulizotaja, ipi ilikuwa changamoto kubwa zaidi?", question_type=FormQuestion.QuestionType.LONG_TEXT, order=4, required=True, existing_label_en="Which was the greatest challenge?")
